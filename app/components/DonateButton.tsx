...
export default function DonateButton({ amount = 500, className = '', hostedLinkFallback, children }: DonateButtonProps) {
  const [loading, setLoading] = useState(false);

  const handleClick = async () => {
    try {
      setLoading(true);
      await openDonationCheckout({ amountInRupees: amount, hostedLinkFallback });
    } catch (e) {
      console.error(e);
